...
SafeOrScam is loaded as an **unpacked extension** in Chrome. The **OpenAI, Exa, and SerpAPI keys come pre-configured**, so there's nothing to sign up for or paste in — just load the folder and go.
...
Open any Carousell listing (a URL with `/p/` in it, e.g. `https://www.carousell.sg/p/...`). Within a couple of seconds the SafeOrScam sidebar slides in from the right with the trust gauge and verdict — no setup required.

### (Optional) Use your own API keys

The extension ships with working keys, so this step is only needed if you'd rather run it on your own accounts (e.g. to avoid shared rate limits). Open the SafeOrScam **Options** page — on the card in `chrome://extensions`, click **Details → Extension options**, or right-click the pinned icon → **Options** — paste your **OpenAI**, **Exa**, and **SerpAPI** keys, and click **Save keys**. Keys entered here are stored locally in `chrome.storage.local`, are only ever sent to their respective provider APIs, and take precedence over the bundled ones.
...
One or more API calls failed. The keys are bundled, so this is usually a rate limit, quota, or temporary network issue rather than a missing key. Open the service-worker console (`chrome://extensions` → SafeOrScam → **Inspect views: service worker**) to see which provider returned an error. The pipeline is resilient — if only one engine fails you'll still get a partial verdict, and "UNAVAILABLE" only appears if all of them fail at once.
